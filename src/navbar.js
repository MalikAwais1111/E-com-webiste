...
import { categorytxt } from "./features2/CategorySlice";
import Category from "./category";

const NavBar = () => {
  const { numb, data,setitmsrch,itmsrch} = useContext(Cartcontext);
 ...
 const cgry = ["smartphones","laptops","fragrances","skincare","groceries","home-decoration","furniture","tops","womens-dresses"];
  ...
  return (
    <div className="navbar">
      <h2 onClick={cchange}><img className="burgur-menu" src="/images/burgur icon.png" alt="" /></h2>
      <div className={cname}>
        <h3 >Catagory</h3> 
          {cgry &&
          cgry.map((c)=>{
      ...
      <div className="logo">
        <img onClick={handlehome} src="/images/EC-website-final-logo-removebg-preview.png" />
      </div>
      { <div className="SearchBar">
        <input onKeyDown={handleKeyPress}
        value={itemtosearch}
        onChange={(e)=>setitemtosearch(e.target.value)}
        type="text" placeholder="Search" />{setitmsrch(itemtosearch)}
        <button className="btnsearch" onClick={handlesearch}><img src="images/searchicon-removebg-preview.png" alt="" /></button>
      </div>}
      <div className="cart" >
        <Link className="homepage" to="/" onClick={hide} >Home</Link>
        <Link to="/cart" onClick={hide}>
          <img src="/images/cart-removebg-preview.png" />
